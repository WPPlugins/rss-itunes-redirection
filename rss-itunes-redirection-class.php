...
class iTunesRSSRedirection {
	...
	public function wp_menu( $id ) {
		add_options_page( __( 'iTunes Redirect', 'rss-itunes-redirection' ), __( 'iTunes Redirect', 'rss-itunes-redirection' ), 'administrator', __FILE__, array(
			$this,
			'settings_page'
		) );
	}
	...
	function wp_settings() {
		...
		add_settings_section( 'rss_itunes_redirect_section', __( 'RSS iTunes Redirection Settings', 'rss-itunes-redirection' ), array(
			$this,
			'sectionText'
		), __FILE__ );
		add_settings_field( 'iTRSSR_url', __( 'New URL to redirect', 'rss-itunes-redirection' ), array(
			$this,
			'iTRSSR_url'
		), __FILE__, 'rss_itunes_redirect_section' );
		add_settings_field( 'iTRSSR_namespace_add', __( 'Add the namespace?', 'rss-itunes-redirection' ), array(
			$this,
	...
	function iTRSSR_namespace_add() {
		...
		echo __( 'Will add the namespace', 'rss-itunes-redirection' ) . ': <strong>xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd"</strong>';
	}
